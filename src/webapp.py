...
def run_pipeline(job_id: str, url: str, goal: str):
    job = jobs[job_id]
    log = []
...
def start():
    ...
    t = threading.Thread(target=run_pipeline, args=(job_id, url, goal))
    t.daemon = True
    t.start()
...
def gallery(job_id):
    ...
    for s in steps:
        ...
        step_data.append({
            "index": s["index"],
            "label": s.get("label") or f"Step {s['index']}",
            "url": s.get("url", ""),
            "filename": p.name,
        })
...
def screenshot(job_id, filename):
    path = PROJECT_DIR / f"output/{job_id}/screenshots/{filename}"
    if not path.exists():
...
def download(job_id):
    ...
    screenshots_dir = PROJECT_DIR / f"output/{job_id}/screenshots"
    zip_path = PROJECT_DIR / f"output/{job_id}/flow-screenshots.zip"

    with zipfile.ZipFile(zip_path, "w") as zf:
        for img in sorted(screenshots_dir.glob("*.png")):
            zf.write(img, img.name)

    return send_file(zip_path, as_attachment=True, download_name="flow-screenshots.zip")
