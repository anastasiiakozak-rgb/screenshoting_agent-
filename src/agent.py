...
RUNS = [
    {
        "id": "cleanmymac-purchase",
        "name": "CleanMyMac Purchase Flow",
        "start_url": "https://cleanmymac.com/",
        "first_click": "a[href*='store/cleanmymac']",
        "first_click_fallback_url": "https://macpaw.com/store/cleanmymac",
        "goal": "Complete the purchase flow: go through plan selection, sign up with email and password, and reach the payment/checkout page. Fill in fake card details to capture the payment form UI but do not submit payment.",
        "max_steps": 15,
    },
]
...
# ------------------------------------------------------------
# ELEMENT FINDER — tries main page then iframes
# ------------------------------------------------------------
...
async def find_element(page, selector):
    ...
    # 2. Try inside iframes (Stripe, Paddle checkout)
    ...
    # 3. Text fallback — a and button only, never div/span
        ...
        for frame in page.frames:
            ...
            for tag in ["a", "button", "input"]:
                try:
                    ...
                    if await el.is_visible(timeout=1000):
                        print(f"  ✅ Found in iframe via text: {tag}:has-text('{text}')")
                        return el
                except Exception:
                    pass
...
Only set is_goal_complete to true after you see a confirmation/success page."""
        ...
        payment_instructions = "If you reach a payment form, set should_stop to true with reason 'reached-payment'."
...
- Tag + attribute: input[type='email'], button[type='submit'], input[name='cardnumber']
...
- NEVER use commas or combine selectors — ONE selector only
...
async def run_agent(run_config, page):
    ...
    if run_config.get("first_click"):
        print(f"\n  🖱️  First click: {run_config['first_click']}")
        ...
        if el:
            ...
            print(f"  ↩️  Falling back to direct URL: {run_config['first_click_fallback_url']}")
    ...
    for step_index in range(run_config["max_steps"]):
        ...
        filepath, screenshot_bytes = await save_screenshot(
            page, output_dir, step_index, f"step-{step_index}"
        )
        ...
        try:
            analysis = ask_claude(
                ...
                email,
                password,
                payment=run_config.get("payment")
            )
        ...
        # Check stop conditions AFTER saving the screenshot
        ...
        if analysis.get("should_stop"):
            ...
            print(f"\n  ⏹️  Stopping: {reason}")
            if reason == "google-only":
                print("  ℹ️  Sign in manually in the browser, then press ENTER.")
                loop = asyncio.get_event_loop()
                await loop.run_in_executor(None, sys.stdin.readline)
                await wait_for_page_settle(page)
                continue
            break  # screenshot already saved above — safe to stop now
        ...
        # Stuck detection — only stop if BOTH url unchanged AND label is same as previous
        if page.url == url_before:
            ...
            if prev_label == curr_label:
                ...
                stuck_count = 0  # URL same but label changed = new step (e.g. Paddle modal)
        else:
            stuck_count = 0
    ...
    print(f"   Output: ./output/{run_config['id']}/screenshots/")
    return steps
